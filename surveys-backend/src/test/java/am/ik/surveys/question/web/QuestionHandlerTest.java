...
class QuestionHandlerTest {
	...
	void setUp(RestDocumentationContextProvider restDocumentation) {
		...
		this.questionChoiceRepository = new QuestionChoiceRepository(databaseClient, transactionalOperator,
				sqlSupplier);
		this.questionRepository = new QuestionRepository(databaseClient, transactionalOperator, sqlSupplier);
		final QuestionHandler questionHandler = new QuestionHandler(this.ulid, questionRepository,
				questionChoiceRepository);

		this.testClient = TestUtils.webTestClient(questionHandler.routes(), restDocumentation).build();
		...
		Fixtures.questionChoices.forEach(
				questionChoice -> StepVerifier.create(this.questionChoiceRepository.insert(Mono.just(questionChoice)))
						.expectNext(questionChoice).verifyComplete());
	}

	@Test
	void getQuestions() {
		this.testClient.get().uri("/questions").headers(basicAdmin()).exchange().expectStatus().isOk()
				...
				.consumeWith(document("get-questions",
						...
						responseFields(fieldWithPath("[].question_id").type(JsonFieldType.STRING).description("設問ID"),
	...
	void postQuestions() {
		...
		this.testClient.post().uri("/questions").headers(basicAdmin()).contentType(MediaType.APPLICATION_JSON)
				...
				.consumeWith(result -> {
					...
					final Mono<Question> questionMono = this.questionRepository
							.findById(Question.Id.valueOf(questionId));
					StepVerifier.create(questionMono).consumeNextWith(question -> {
						assertThat(question.getQuestionId().toString()).isEqualTo(questionId);
						assertThat(question.getQuestionText()).isEqualTo("自由回答の質問");
					}).expectComplete().verify();
				})
				.consumeWith(document("post-questions",
						...
						requestFields(fieldWithPath("question_text").type(JsonFieldType.STRING).description("設問文")),
						responseHeaders(headerWithName(CONTENT_TYPE).description(APPLICATION_JSON_VALUE),
	...
	void postSelectiveQuestions() {
		...
		this.testClient.post().uri("/questions").contentType(MediaType.APPLICATION_JSON).headers(basicAdmin())
				.bodyValue(requestBody).exchange().expectStatus().isCreated().expectBody(JsonNode.class)
				.consumeWith(result -> {
					final JsonNode res = result.getResponseBody();
					assertThat(res).isNotNull();
				...
				.consumeWith(document("post-selective-questions",
						requestHeaders(headerWithName(CONTENT_TYPE).description(APPLICATION_JSON_VALUE),
								headerWithName(AUTHORIZATION).description("Basic認証")),
						requestFields(fieldWithPath("question_text").type(JsonFieldType.STRING).description("設問文"),
								fieldWithPath("max_choices").type(JsonFieldType.NUMBER).description("選択可能数")),
	...
	void getQuestion() {
		...
		this.testClient.get().uri("/questions/{question_id}", question.getQuestionId()).exchange().expectStatus().isOk()
				.expectBody(JsonNode.class).consumeWith(result -> {
					final JsonNode res = result.getResponseBody();
					assertThat(res).isNotNull();
				...
				.consumeWith(document("get-question",
						...
						responseFields(fieldWithPath("question_id").type(JsonFieldType.STRING).description("設問ID"),
								...
								fieldWithPath("question_choices").type(JsonFieldType.ARRAY).description("設問選択肢")
										.optional(),
								fieldWithPath("question_choices[].question_choice_id").type(JsonFieldType.STRING)
										.description("設問選択肢ID").optional(),
								fieldWithPath("question_choices[].question_id").type(JsonFieldType.STRING)
										.description("設問ID").optional(),
								fieldWithPath("question_choices[].question_choice_text").type(JsonFieldType.STRING)
										.description("選択肢本文").optional(),
								fieldWithPath("question_choices[].allow_free_text").type(JsonFieldType.BOOLEAN)
										.description("自由記述可").optional())));
	}

	@Test
	void getQuestionChoices() {
		final Question question = Fixtures.questions.get(0);
		this.testClient.get().uri("/questions/{question_id}/question_choices", question.getQuestionId()).exchange()
				...
				.consumeWith(document("get-question-question-choices",
						pathParameters(parameterWithName("question_id").description("設問ID")),
						responseHeaders(headerWithName(CONTENT_TYPE).description(APPLICATION_JSON_VALUE)),
						responseFields(
								fieldWithPath("[].question_choice_id").type(JsonFieldType.STRING)
										.description("設問選択肢ID"),
								fieldWithPath("[].question_id").type(JsonFieldType.STRING).description("設問ID"),
								fieldWithPath("[].question_choice_text").type(JsonFieldType.STRING)
										.description("選択肢本文"),
	...
	void postQuestionChoices() {
		...
		this.testClient.post().uri("/questions/{question_id}/question_choices", question.getQuestionId())
				.contentType(MediaType.APPLICATION_JSON).headers(basicAdmin()).bodyValue(requestBody).exchange()
				.expectStatus().isCreated().expectBody(JsonNode.class).consumeWith(result -> {
					final JsonNode res = result.getResponseBody();
					assertThat(res).isNotNull();
					final String questionId = res.get("question_id").asText();
					...
					StepVerifier.create(questionChoiceMono).consumeNextWith(questionChoice -> {
				...
				.consumeWith(document("post-question-question-choices",
						requestHeaders(headerWithName(CONTENT_TYPE).description(APPLICATION_JSON_VALUE),
								headerWithName(AUTHORIZATION).description("Basic認証")),
						requestFields(
