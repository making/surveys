...
class SurveyQuestionHandlerTest {
	...
	void setUp(RestDocumentationContextProvider restDocumentation) {
		...
		final SurveyRepository surveyRepository = new SurveyRepository(databaseClient, transactionalOperator,
				sqlSupplier);
		final SurveyQuestionHandler surveyQuestionHandler = new SurveyQuestionHandler(surveyRepository,
				surveyQuestionRepository, questionRepository, questionChoiceRepository);
		this.testClient = TestUtils.webTestClient(surveyQuestionHandler.routes(), restDocumentation).build();
	...
	void getSurveyQuestions() {
		...
		this.testClient.get().uri("/surveys/{survey_id}/survey_questions", survey.getSurveyId()).headers(basicAdmin())
				.exchange().expectStatus().isOk().expectBody(JsonNode.class).consumeWith(result -> {
					final JsonNode res = result.getResponseBody();
					assertThat(res).isNotNull();
					assertThat(res).hasSize(3);
				})
				.consumeWith(document("get-survey-questions",
						...
						responseFields(fieldWithPath("[].required").type(JsonFieldType.BOOLEAN).description("回答必須"),
	...
	void deleteSurveyQuestion() {
		...
		this.testClient.delete()
				.uri("/surveys/{survey_id}/survey_questions/{question_id}", surveyQuestion.getSurveyId(),
						surveyQuestion.getQuestionId())
				.headers(basicAdmin()).exchange().expectStatus().isNoContent().expectBody()
				.consumeWith(document("delete-survey-question",
						requestHeaders(headerWithName(AUTHORIZATION).description("Basic認証")),
						pathParameters(parameterWithName("survey_id").description("アンケートID"),
		...
		final Mono<Boolean> isDeleted = this.surveyQuestionRepository.findBySurveyId(surveyQuestion.getSurveyId())
				.filter(x -> Objects.equals(x.getQuestionId(), surveyQuestion.getQuestionId())).collectList()
				.map(List::isEmpty);
		StepVerifier.create(isDeleted).expectNext(true).expectComplete().verify();
	}
	...
	void postSurveyQuestion() {
		...
		final Question question = new Question.Builder().withQuestionId(questionId).withQuestionText("その他コメント").build();
		final Map<String, Boolean> requestBody = Map.of("required", false);
		StepVerifier.create(this.questionRepository.insert(Mono.just(question))).expectNext(question).expectComplete()
				.verify();
		this.testClient.post()
				...
				.consumeWith(document("post-survey-questions",
						...
						pathParameters(parameterWithName("survey_id").description("アンケートID"),
								parameterWithName("question_id").description("設問ID")),
						requestFields(fieldWithPath("required").type(JsonFieldType.BOOLEAN).description("回答必須"))));
		final Mono<Boolean> isCreated = this.surveyQuestionRepository.findBySurveyId(survey.getSurveyId())
				.filter(surveyQuestion -> Objects.equals(surveyQuestion.getQuestionId(), questionId)).collectList()
				.map(l -> l.size() == 1);
		StepVerifier.create(isCreated).expectNext(true).expectComplete().verify();
	}
}