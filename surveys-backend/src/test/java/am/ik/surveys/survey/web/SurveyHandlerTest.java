...
class SurveyHandlerTest {
	...
	void setUp(RestDocumentationContextProvider restDocumentation) {
		...
		final QuestionRepository questionRepository = new QuestionRepository(databaseClient, transactionalOperator,
				sqlSupplier);
		final SurveyQuestionRepository surveyQuestionRepository = new SurveyQuestionRepository(databaseClient,
				transactionalOperator, sqlSupplier);
		...
		this.surveyQuestionRepository = new SurveyQuestionRepository(databaseClient, transactionalOperator,
				sqlSupplier);
		final AnswerRepository answerRepository = new AnswerRepository(databaseClient, transactionalOperator,
				sqlSupplier);
		final AnswerDetailRepository answerDetailRepository = new AnswerDetailRepository(databaseClient,
				transactionalOperator, sqlSupplier);
		final SurveyHandler surveyHandler = new SurveyHandler(this.ulid, surveyRepository, surveyQuestionRepository,
				questionRepository, questionChoiceRepository, answerRepository, answerDetailRepository,
				transactionalOperator);
		this.testClient = TestUtils.webTestClient(surveyHandler.routes(), restDocumentation).build();

		Fixtures.surveys.forEach(survey -> StepVerifier.create(this.surveyRepository.insert(Mono.just(survey)))
		...
		Fixtures.surveyQuestions
				.forEach(surveyQuestion -> StepVerifier.create(this.surveyQuestionRepository.insert(surveyQuestion))
						.expectNext(surveyQuestion).verifyComplete());
	}

	@Test
	void deleteSurvey() {
		final Survey survey = Fixtures.surveys.get(0);
		this.testClient.delete().uri("/surveys/{survey_id}", survey.getSurveyId()).headers(basicAdmin()).exchange()
				.expectStatus().isNoContent().expectBody().consumeWith(
						document("delete-survey", requestHeaders(headerWithName(AUTHORIZATION).description("Basic認証")),
		...
		StepVerifier.create(surveyMono).expectComplete().verify();
	}

	@Test
	void getSurvey() {
		final Survey survey = Fixtures.surveys.get(0);
		this.testClient.get().uri("/surveys/{survey_id}", survey.getSurveyId()).exchange().expectStatus().isOk()
				.expectBody(JsonNode.class).consumeWith(result -> {
					...
					assertThat(OffsetDateTime.parse(res.get("start_date_time").asText()))
							.isEqualTo(survey.getStartDateTime());
					assertThat(OffsetDateTime.parse(res.get("end_date_time").asText()))
							.isEqualTo(survey.getEndDateTime());
					final JsonNode surveyQuestions = res.get("survey_questions");
					assertThat(surveyQuestions).hasSize(3);
				})
				.consumeWith(document("get-survey",
						pathParameters(parameterWithName("survey_id").description("アンケートID")),
						responseHeaders(headerWithName(CONTENT_TYPE).description(APPLICATION_JSON_VALUE)),
						responseFields(fieldWithPath("survey_id").type(JsonFieldType.STRING).description("アンケートID"),
								...
								fieldWithPath("survey_questions[].required").type(JsonFieldType.BOOLEAN)
										.description("回答必須"),
								fieldWithPath("survey_questions[].question_id").type(JsonFieldType.STRING)
										.description("設問ID"),
								fieldWithPath("survey_questions[].survey_id").type(JsonFieldType.STRING)
										.description("アンケートID"),
								fieldWithPath("survey_questions[].question_text").type(JsonFieldType.STRING)
										.description("設問文"),
								fieldWithPath("survey_questions[].max_choices").type(JsonFieldType.NUMBER)
										.description("選択可能数").optional(),
								fieldWithPath("survey_questions[].question_choices").type(JsonFieldType.ARRAY)
										.description("設問選択肢").optional(),
								fieldWithPath("survey_questions[].question_choices[].question_choice_id")
										.type(JsonFieldType.STRING).description("設問選択肢ID").optional(),
								fieldWithPath("survey_questions[].question_choices[].question_id")
										.type(JsonFieldType.STRING).description("設問ID").optional(),
								fieldWithPath("survey_questions[].question_choices[].question_choice_text")
										.type(JsonFieldType.STRING).description("選択肢本文").optional(),
								fieldWithPath("survey_questions[].question_choices[].allow_free_text")
										.type(JsonFieldType.BOOLEAN).description("自由記述可").optional())));
	}

	@Test
	void getSurveys() {
		final Survey survey1 = Fixtures.surveys.get(0);
		this.testClient.get().uri("/surveys").headers(basicAdmin()).exchange().expectStatus().isOk()
				.expectBody(JsonNode.class).consumeWith(result -> {
					final JsonNode res = result.getResponseBody();
					assertThat(res).isNotNull();
					...
					assertThat(OffsetDateTime.parse(res.get(0).get("start_date_time").asText()))
							.isEqualTo(survey1.getStartDateTime());
					assertThat(OffsetDateTime.parse(res.get(0).get("end_date_time").asText()))
							.isEqualTo(survey1.getEndDateTime());
				}).consumeWith(document("get-surveys",
	...
	void postSurveys() {
		...
		final Map<String, Object> requestBody = Map.of("survey_title", "テストアンケート", "start_date_time",
				startDateTime.toString(), "end_date_time", endDateTime.toString());

		this.testClient.post().uri("/surveys").contentType(MediaType.APPLICATION_JSON).headers(basicAdmin())
				.bodyValue(requestBody).exchange().expectStatus().isCreated().expectBody(JsonNode.class)
				.consumeWith(result -> {
					final JsonNode res = result.getResponseBody();
					assertThat(res).isNotNull();
					...
					StepVerifier.create(surveyMono).consumeNextWith(survey -> {
				...
				.consumeWith(document("post-surveys",
						requestHeaders(headerWithName(CONTENT_TYPE).description(APPLICATION_JSON_VALUE),
								headerWithName(AUTHORIZATION).description("Basic認証")),
						requestFields(fieldWithPath("survey_title").type(JsonFieldType.STRING).description("アンケートタイトル"),
								fieldWithPath("start_date_time").type(JsonFieldType.STRING).description("開始予定時刻"),
								fieldWithPath("end_date_time").type(JsonFieldType.STRING).description("終了予定時刻")),
						responseHeaders(headerWithName(CONTENT_TYPE).description(APPLICATION_JSON_VALUE),
